...
try {
    ...
    $order_status = isset($data['send_to_kitchen']) && $data['send_to_kitchen'] ?  'preparing' : 'pending';
    $payment_status = isset($data['payment_method']) ? 'paid' : 'unpaid';
    ...
    if ($payment_status == 'paid') {
        $query = "INSERT INTO payments (order_id, amount, payment_method, cashier_id)
                  VALUES (:order_id, :amount, :payment_method, :cashier_id)";
        $stmt = $db->prepare($query);
        $stmt->bindParam(':order_id', $order_id);
        $stmt->bindParam(':amount', $data['total']);
